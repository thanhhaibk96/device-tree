...
static int mcp251x_do_set_bittiming(struct net_device *net)
{
	...
	printk(KERN_INFO "mcp251x: CAN Baudrate: %d MHz\n",  bt->bitrate);
/* 
  ...
  */
	...
	mcp251x_write_reg(spi, CNF1, cfg1);
	mcp251x_write_reg(spi, CNF2, cfg2);
	mcp251x_write_reg(spi, CNF3, cfg3);
	...
	mcp251x_write_reg(spi, CNF2, CNF2_BTLMODE |
			  ...
			  (bt->prop_seg - 1));
	mcp251x_write_reg(spi, CNF3, 0x75);*/
...
static int mcp251x_hw_probe(struct spi_device *spi)
{
	...
	printk(KERN_INFO "mcp251x: CANCTRL: %x\n",  ctrl);
...
static int mcp251x_open(struct net_device *net)
{
	...
	printk(KERN_INFO "mcp251x: IRQ number: %d\n", spi->irq);
...
static int mcp251x_can_probe(struct spi_device *spi)
{
	...
	printk(KERN_INFO "mcp251x: This is my probe function\n");
	...
	printk(KERN_INFO "mcp251x: MCP2515 Oscillator_frequency: %d\n",  freq);	
	...
	printk(KERN_DEBUG "mcp251x: Module CAN model: MCP%x\n",  priv->model);	
	...
	printk(KERN_INFO "mcp251x: SPI Clock %d\n",  spi->max_speed_hz);	
	printk(KERN_INFO "mcp251x: SPI Mode %x\n",  spi->mode);	
	printk(KERN_INFO "mcp251x: Enable DMA: %x\n", mcp251x_enable_dma);
	...
	printk(KERN_INFO "mcp251x: MCP2515 successfully initialized\n");	
...
MODULE_AUTHOR("Thanh Hai Chau <thanhhaipif96@gmail.com>");
